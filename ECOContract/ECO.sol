...
contract EcoContract is PluginClient, Ownable {
   ...
   enum InsurancePeriod {
    ...
    uint256 public constant ORACLE_PAYMENT = 0.001 * 10 ** 18;
    ...
    function addVehicles(uint256 _vehicleNo, uint256 _obdId, string memory _carbonHash) public onlyOwner {
        vehicles[vehicleid]  = VehicleInfo(vehicleid,_obdId,_vehicleNo,msg.sender,_carbonHash);
        vehicleid = vehicleid + 1;
    }

    function dailyData(uint256 _vehicleNo, uint256 _obdId, string memory _carbonHash) public onlyOwner {
        vehicles[vehicleid]  = VehicleInfo(vehicleid,_obdId,_vehicleNo,msg.sender,_carbonHash);
        vehicleid = vehicleid + 1;
    }
